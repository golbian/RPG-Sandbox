angular.module('app').controller('ioCtrl', function ($scope, $q, connection, $routeParams, ioModel, FileSaver, Upload) {
    ...
    $scope.initExport = function () {
        return $q.all([
            ...
            ioModel.getMaps().then(maps => { $scope.maps = maps; }),
            ioModel.getGames().then(games => { $scope.games = games; }),
        ]);
    };
    ...
    $scope.downloadExport = function () {
        ...
        const mapIDs = getCheckedIds($scope.maps);
        const gameIDs = getCheckedIds($scope.games);

        ioModel.makeExportBundle(gameIDs, mapIDs, layerIDs)
            .then(bundle => {
    ...
    $scope.upload = function (file) {
        ...
        $scope.fileReader.onload = function () {
            try {
                ...
                if (!($scope.importFile.layers && $scope.importFile.maps &&
                    $scope.importFile.datasources && $scope.importFile.games)) {
