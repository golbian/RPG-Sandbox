(function () {
    ...
    function UsersViewController ($routeParams, $uibModal, connection, userService, gettextCatalog) {
        ...
        function activate () {
            ...
            connection.get('/api/get-user-maps/' + $routeParams.userID, { userID: $routeParams.userID }).then(function (data) {
                vm.userMaps = data.items;
            });

            connection.get('/api/get-user-games/' + $routeParams.userID, { userID: $routeParams.userID }).then(function (data) {
                vm.userGames = data.items;
            });
        };
        ...
        function loadRoles () {
            connection.get('/api/roles/find-all', {}).then(function (data) {
                ...
                var adminRole = { _id: 'WSTADMIN', name: 'RPG-Sandbox Administrator' };
        ...
        function deleteRole (roleID) {
            if (vm.user.userName === 'administrator' && roleID === 'WSTADMIN') {
                noty({ text: "The role 'RPG-Sandbox Administrator' can't be removed from the user administrator", timeout: 6000, type: 'warning' });
            } else {
                $uibModal.open({
